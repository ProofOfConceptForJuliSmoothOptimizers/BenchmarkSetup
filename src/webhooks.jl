...
function is_new_webhook(
    ...
    try
        response =
            GitHub.gh_get_json(api, "/repos/$org/$(repository.name)/hooks"; kwargs...)
...
function create_benchmark_webhook(
    api::GitHub.GitHubWebAPI,
    org::String,
    repository::Repo;
    kwargs...,
    ...
    webhook_config = Dict(
        :url => "$(PAYLOAD_URL)$(format_repository_name(repository))",
        :content_type => "json",
        :insecure_ssl => "0",
        :active => true,
    )
    ...
    if is_new_webhook(api, org, repository, webhook_config; kwargs...)
        ...
        response = GitHub.gh_post_json(
            api,
            "/repos/$org/$(repository.name)/hooks";
            params = myparams,
            kwargs...,
        )
